...
p=p %>%
  add_trace(type="scatter", mode="lines", x=df[["Date"]], y=df[["Subways..Total.Estimated.Ridership"]], hovertemplate="%{y:,.0f}") %>%
  layout(title="Daily Subway Ridership During COVID-19", xaxis=list(title="Date"), yaxis=list(title="Ridership")) %>%
