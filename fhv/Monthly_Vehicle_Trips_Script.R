...
p=p %>%
  ...
  add_trace(type="scatter", 
            ...
            name = "Yellow",
            hovertemplate ="%{y:,.0f}") %>%
  layout(title = "<b>NYCTLC Monthly Vehicle Trips, by Vehicle License Type, 2015-2021</b>",
         xaxis = list(title="<b>Month/Year</b>"),
         yaxis = list(title="<b>Number of Trips</b>")) %>%
  config(displayModeBar=F) %>%
  layout(xaxis=list(showgrid=FALSE)) %>%
  layout(annotations=list(text='Data Source: <a href="https://www1.nyc.gov/site/tlc/about/aggregated-reports.page", target="blank">NYCTLC</a> | <a href ="https://raw.githubusercontent.com/NYCPlanning/td-trends/main/fhv/TLC_Monthly_Ridership_EstimatedTotals.csv" target="blank">Download Chart Data</a>',
                          font=list(size=11),
                          showarrow=F,
                          x=1,
                          xanchor='right',
