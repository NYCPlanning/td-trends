...
df=read.csv(paste0(path,'fhv/TLC_Monthly_Ridership.csv'),stringsAsFactors=F,check.names=F)
df=df %>%
...
p=p %>%
  layout(template='plotly_white',
         title=list(text=paste0('<b>TLC Monthly Ridership</b>'),
                    font=list(size=20),
                    x=0.5,
                    xanchor='center',
                    y=0.95,
         ...
         hovermode='x unified')
p=p %>% 
  add_annotations(text='Data Source: <a href="https://www1.nyc.gov/site/tlc/about/aggregated-reports.page" target="blank">NYC TLC</a> | <a href="https://raw.githubusercontent.com/NYCPlanning/td-trends/main/fhv/TLC_Monthly_Ridership.csv" target="blank">Download Chart Data</a>',
                  font=list(size=14),
                  showarrow=F,
                  x=1,
                  xanchor='right',
...
p
htmlwidgets::saveWidget(p,paste0(path,'fhv/TLC_Monthly_Ridership.html'))
