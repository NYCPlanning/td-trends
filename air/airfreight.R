...
df=read.csv(paste0(path,'air/airfreight.csv'),stringsAsFactors=F,check.names=F)
...
p=p %>%
  add_trace(type='scatter',
            ...
            y=df[['LGA Int.']],
            showlegend=F,
            hovertext=paste0('<b>Total Short Tons: </b>',format(df[['Total']],trim=T,big.mark=',')),
            hoverinfo='text')
p=p %>%
  layout(template='plotly_white',
         title=list(text=paste0('<b>Airport Revenue Freight</b>'),
                    font=list(size=20),
                    x=0.5,
                    xanchor='center',
                    y=0.95,
         ...
         xaxis=list(title=list(text='<b>Year</b>',
                    ...
                    fixedrange=T,
                    showgrid=F),
         yaxis=list(title=list(text='<b>Short Tons</b>',
                               font=list(size=14)),
                    tickfont=list(size=12),
                    rangemode='tozero',
                    fixedrange=T,
         ...
         hovermode='x unified')
p=p %>% 
  add_annotations(text='Data Source: <a href="https://www.panynj.gov/airports/en/statistics-general-info.html" target="blank">PANYNJ</a> | <a href="https://raw.githubusercontent.com/NYCPlanning/td-trends/main/air/airfreight.csv" target="blank">Download Chart Data</a>',
                  font=list(size=14),
                  showarrow=F,
                  x=1,
                  xanchor='right',
...
p
htmlwidgets::saveWidget(p,paste0(path,'air/airfreight.html'))
