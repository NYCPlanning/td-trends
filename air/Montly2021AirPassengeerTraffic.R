...
fig <- plot_ly(df, x= ~Month, y= ~EWR, type = "bar", name = "EWR", marker = list(color = "#1b9e77"))
fig <- fig %>% add_trace(y = ~JFK, name = "JFK", marker = list(color="#d95f02"))
fig <- fig %>% add_trace(y = ~LGA, nane = "LGA", marker = list(color="#7570b3"))
