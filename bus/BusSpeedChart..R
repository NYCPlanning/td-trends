...
df=read.csv(paste0(path,'bus/BusSpeed2020-2021.csv'),stringsAsFactors=F,check.names=F)
...
p=p %>%
  add_trace(type='scatter',
            ...
            y=df[['Manhattan']],
            showlegend=F,
            hovertext=paste0('<b>Month: </b>',df[['Year']]),
            hoverinfo='text')
  ...
  p=p %>%
    add_trace(name=i,
              ...
              marker=list(color=dfcolors[i],
                          size=6),
              showlegend=T,
              hovertext=paste0('<b>',i,': </b>',round(df[[i]],trim=T,big.mark=',')),
              hoverinfo='text')
}
p=p %>%
  add_trace(type='scatter',
